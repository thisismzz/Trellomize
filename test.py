...
class TestMainClsUser(TestCase):
    ...
    def test_user_file_validation(self):
        ...
        expected_data_user1 = {"email": "user1@test.com", "username": "user1test",
                               "password": "user1", "active": True, "ID": "tester"}
        self.assertTrue(os.path.exists(
            f"users/{self.user1.username}/{self.user1.username}.json"))
        self.assertEqual(expected_data_user1,
                         User.load_user_data(self.user1.username))
...
class TestMainClsProject(TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.project1 = Project("project test1", "user1",
                               collaborators=None, ID="tester")
    ...
    def test_project_file_validation(self):
        ...
        expected_data_project = {"title": "project test1", "owner": "user1", "tasks": {
        }, "collaborators": ["user1"], "ID": "tester"}
        self.assertTrue(os.path.exists("projects/tester.json"))
        self.assertEqual(expected_data_project,
                         Project.load_project_data("tester"))


class TestMainClsTask(TestCase):

    def setUp(self):
        self.task1 = Task("task1 test", "test No.1", ID="tester",
                          start_time="2024-05-22 22:56:04.092445", end_time="2024-05-23 22:56:04.092445")
