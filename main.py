...
class User:
    ...
    def change_username(self, new_username):
        ...
        try:
            ...
            logger.info(f"User [{self.username}] changed username from {
    ...
    def change_password(self, new_password):
        if bcrypt.checkpw(new_password.encode('utf-8'), self.password.encode('utf-8')):
            console.print(
                "Enter a new password not your old password!", style="Error")
            return
        self.password = bcrypt.hashpw(new_password.encode(
    ...
    def change_email(self, new_email):
        if new_email == self.email:
            console.print(
                "Enter a new email not your old email!", style='Error')
            return
        ...
        try:
            ...
            logger.info(f"User [{self.username}] changed email from {
    ...
    def edit_profile_menu(self):
        while True:
            ...
            if choice == "1":
                ...
                new_username = input(
                    "Enter new username (or press ENTER to go back): ")
                if new_username == "":
                    return
                if User.check_unique_username(new_username):
                    ...
                    console.print(
                        "Username already exists. Please choose a different one.", style="Error")
                    wait_for_key_press()
            elif choice == "2":
                clear_screen()
                console.print("|Editing Password|\n", style="Title")
                new_password = input(
                    "Enter new password (or press ENTER to go back): ")
                if new_password == "":
                    return
                ...
                new_email = input(
                    "Enter new email (or press ENTER to go back): ")
                if new_email == "":
                    return
                if User.check_unique_email(new_email):
                    ...
                    console.print(
                        "Email already exists. Please choose a different one.", style="Error")
                    wait_for_key_press()
            elif choice == "4":
                break
            else:
    ...
    def register():
        ...
        console.print(
            "Please provide the following details to create your account:", style="Info")
        ...
        try:
            if not User.validate_email_format(email):
                raise ValueError(
                    "Invalid email format! Please enter a valid email address in the format 'example@example.com'.")
    ...
    def login():
        ...
        console.print("|Login|\n", style="Title")
        console.print(
            "Please provide your credentials to log in:", style="Info")
        console.print("(or press ENTER to go back)", style='Info')
        username = input("Username: ")
        if username == "":
            return
        # Using maskpass to hide password input with '*'
...
class Task:
    ...
    def __init__(self, title, description, priority=None, status=None, ID=None, start_time=None, end_time=None, assignees=[], comments=[], history=[]):
        ...
        self.ID = ID if ID is not None else str(uuid.uuid1())[:8]
        self.start_time = start_time if start_time is not None else str(
            datetime.now())
        self.end_time = end_time if end_time is not None else str(
    ...
    def view_task(self):
        ...
        table.add_row(self.ID, self.title, self.description,
                      self.start_time[:19], self.end_time[:19], self.priority, self.status)
        console.print(table)
    ...
    def change_end_time(self):
        ...
        new_end_time = input(
            "Enter new end time (YYYY-MM-DD HH:MM:SS): (or press ENTER to go back)")
        if new_end_time == "":
            return False
        try:
            self.end_time = str(datetime.strptime(
                new_end_time, "%Y-%m-%d %H:%M:%S"))
            console.print("End time changed successfully.", style="Notice")
            logger.debug(
                f"Task [id : {self.ID}] end time has changed (current : {self.end_time})")
            wait_for_key_press()
            return True
        except ValueError:
            console.print(
                "Invalid date/time format. Please use the format YYYY-MM-DD HH:MM:SS.", style="Error")
            return False
            wait_for_key_press()
    ...
    def change_start_time(self):
        ...
        new_start_time = input(
            "Enter new start time (YYYY-MM-DD HH:MM:SS): (or press ENTER to go back)")
        if new_start_time == "":
            return False
        try:
            self.start_time = str(datetime.strptime(
                new_start_time, "%Y-%m-%d %H:%M:%S"))
            console.print("Start time changed successfully.", style="Notice")
            logger.debug(
                f"Task [id : {self.ID}] start time has changed (current : {self.start_time})")
            wait_for_key_press()
            return True
        except ValueError:
            console.print(
    ...
    def change_status(self):
        ...
        new_status = input(
            "Enter the number for the new status or press ENTER to go back: ")
        if new_status == "":
            return False
        ...
        if 0 <= new_status_idx < len(Status):
            ...
            logger.debug(
                f"Task [id : {self.ID}] status has changed (current : {self.status})")
            wait_for_key_press()
            return True
        else:
    ...
    def change_priority(self):
        ...
        new_priority = input(
            "Enter the number for the new priority or press ENTER to go back: ")
        if new_priority == "":
            return False
        ...
        if 0 <= new_priority_idx < len(Priority):
            ...
            console.print("Task priority changed successfully.",
                          style="Notice")
            logger.debug(
                f"Task [id : {self.ID}] priority has changed (current : {self.priority})")
            wait_for_key_press()
            return True
        else:
    ...
    def change_title(self):
        ...
        logger.debug(
            f"Task [id : {self.ID}] title has changed (current : {self.title})")
        wait_for_key_press()
        return True
    ...
    def change_description(self):
        ...
        new_description = input(
            "Enter new description: (or press ENTER to go back)")
        if new_description == "":
            return False
        self.description = new_description
        console.print(f"Task description changed to {
                      self.description}", style="Notice")
        logger.debug(
            f"Task [id : {self.ID}] description has changed (current : {self.description})")
        wait_for_key_press()
        return True
    ...
    def view_comments(self):
        if not self.comments:
            console.print(
                "No comments available for this task to display.", style="Error")
        else:
            clear_screen()
            ...
            table.add_column("Username", justify="center",
                             style="cyan", no_wrap=True, width=15)
            table.add_column("Role", justify="center",
                             style="magenta", width=15)
            table.add_column("Comment", justify="center", style="green")
            table.add_column("Timestamp", justify="center",
                             style="yellow", width=25)

            for idx, comment in enumerate(self.comments, start=1):
                table.add_row(str(idx), get_username(
                    comment['user']), comment['role'], comment['comment'], comment['timestamp'])
    ...
    def add_comment(self, userID, is_owner: bool):
        ...
        if new_comment == "":
            return False
        self.comments.append({"user": userID, "comment": new_comment,
                             "role": "owner" if is_owner else "assignee", "timestamp": str(datetime.now())[:19]})
        console.print("Comment added successfully.", style="Notice")
        logger.debug(f"A new comment added to task [id : {
                     self.ID}] by user [{get_username(userID)}]")
        wait_for_key_press()
        return True
    ...
    def remove_comment(self, user: User):
        if not self.comments:
            console.print(
                "No comments available for this task to remove.", style="Error")
            wait_for_key_press()
            return False
        ...
        try:
            ...
            self.view_comments()
            comment_idx = int(
                input("Enter the number of the comment to remove or press ENTER to go back: "))
            if comment_idx == "":
                return False
            ...
            if 0 <= comment_idx < len(self.comments):
                if self.comments[comment_idx]["user"] == user.ID:
                    ...
                    console.print(f"Comment by {get_username(
                        removed_comment['user'])} removed successfully.", style="Notice")
                    logger.debug(f"Comment removed from task [id : {self.ID}] by user [{
                                 get_username(removed_comment['user'])}]")
                    wait_for_key_press()
                    return True
                else:
                    console.print(
                        "This comment is not belong to you.", style="Error")
                    wait_for_key_press()
                    return False
            else:
            ...
            console.print(
                "Invalid input. Please enter a number.", style="Error")
            wait_for_key_press()
            return False
    ...
    def edit_comment(self, user: User):
        if not self.comments:
            console.print(
                "No comments available for this task to edit.", style="Error")
            wait_for_key_press()
            return False
        ...
        try:
            ...
            self.view_comments()
            comment = int(
                input("Enter the number of the comment to edit or press Enter to go back: "))
            if comment == "":
                return False
            ...
            if 0 <= comment_idx < len(self.comments):
                if self.comments[comment_idx]["user"] == user.ID:
                    ...
                    self.comments[comment_idx]['timestamp'] = str(datetime.now())[
                        :19]
                    console.print("Comment edited successfully.",
                                  style="Notice")
                    logger.debug(f"Comment edited on task [id : {self.ID}] by user [{
                                 get_username(self.comments[comment_idx]['user'])}]")
                    wait_for_key_press()
                    return True
                else:
                    console.print(
    ...
    def add_to_history(self, ID, action, message=None, members=None, new_amount=None):
        if action == "add comment":
            new_history = {"user": ID, "action": action,
    ...
    def view_history(self):
        ...
        for index, entry in enumerate(self.history, start=1):
            ...
            amount = entry.get("new status", "") or entry.get("new priority", "") or entry.get("new start time", "") or entry.get(
                "new end time", "") or entry.get("new title", "") or entry.get("new description", "") or entry.get("message", "")[:20] or entry.get("description")
            if amount == None:
                amount = str(
                    list(map(lambda x: get_username(x), entry.get("new assignees", ""))))
            if amount == "[]":
                amount = str(list(map(lambda x: get_username(
                    x), entry.get("removed assignees", ""))))
...
class Project:
    ...
    def view_members(self):
        if len(self.collaborators) == 1:
            console.print(
                "There are no project members to display.", style="Error")
            wait_for_key_press()
            return
        clear_screen()
    ...
    def add_member(self, member):
        if member not in self.collaborators:
            ...
            console.print(f"Member '{get_username(
                member)}' added to project successfully.", style="Notice")
            User.add_my_project(get_username(member), self.ID)
            self.save_project_data()
            logger.debug(f"A new member [user : {get_username(
                member)}] added to project [id : {self.ID}] collaborators by owner")
        else:
            console.print(f"User {get_username(member)
                                  } has already been added", style='Error')
    ...
    def remove_member(self, user_ID):
        if user_ID in self.collaborators:
            ...
            self.save_project_data()
            console.print(f"Member '{get_username(
                user_ID)}' removed from project successfully.", style="Notice")
            logger.debug(f"A member [user : {get_username(
                user_ID)}] removed from project [id : {self.ID}] collaborators")
        else:
            console.print(
                f"{get_username(user_ID)} is not a member of the project.", style="Error")

    def add_member_menu(self, user: User):
        if self.owner != user.ID:
            console.print(
                "Only the project owner can add members.", style="Error")
            return

        all_usernames = User.get_all_usernames()
        if not all_usernames:
            console.print(
                "There are no available users to add to the project.", style="Error")
            return
        ...
        selected_indices = list(map(lambda x: x.strip(), input(
            "Enter the numbers of the users to add as members (e.g.:'1,2') or press ENTER to go back: ").split(',')))
        ...
        for idx_str in selected_indices:
            if not idx_str.isdigit():
                console.print(
                    "Invalid input. Please enter valid user numbers.", style="Error")
                continue
            idx = int(idx_str) - 1
            if idx >= 0 and idx < len(all_usernames):
                ...
                console.print(f"Invalid user number: {
                              idx + 1}.", style="Error")

    def remove_member_menu(self, user: User):
        if self.owner != user.ID:
            console.print(
                "Only the project owner can remove members.", style="Error")
            return

        members_to_display = [get_username(
            member) for member in self.collaborators if member != self.owner]
        if not members_to_display:
            console.print(
                "There are no project members to remove.", style="Error")
            return
        ...
        selected_indices = list(map(lambda x: x.strip(), input(
            "Enter the numbers of the users to remove from the project (e.g.:'1,2') or press ENTER to go back: ").split(',')))
        if selected_indices[0] == "":
            return
    ...
    def view_assignees(self, task: Task):
        if not task.assignees:
            console.print(
                "There are no assignees for this task.", style="Error")
            wait_for_key_press()
            return
        clear_screen()
    ...
    def assign_member(self, member, task: Task):
        if member in self.collaborators:
            if member not in task.assignees:
                ...
                console.print(
                    f"Member ({get_username(member)}) assigned to task successfully.", style="Notice")
                self.save_project_data()
                logger.debug(f"A new assignee [user : {
                             get_username(member)}] added to task.")
            else:
                console.print(f"Member ({get_username(
                    member)}) is already assigned to the task.", style="Error")
        else:
            console.print(
                f"User ({get_username(member)}) is not a member of the project.", style="Error")

    def remove_assignee(self, userID, task: Task):
        if userID in task.assignees:
            task.assignees.remove(userID)
            console.print(f"Member '{get_username(
                userID)}' removed from task successfully.", style="Notice")
            self.save_project_data()
            logger.debug(
                f"An assignee [user : {get_username(userID)}] removed from task.")
        else:
            console.print(f"Member '{get_username(
                userID)}' is not assigned to the task.", style="Error")

    def assign_member_menu(self, task: Task, user: User):
        if user.ID != self.owner:
            console.print(
                "Only the project owner can assign members.", style="Error")
            return
        if len(self.collaborators) == 1:
            console.print(
                "There are no project members to assign.", style="Error")
            wait_for_key_press()
            return
        clear_screen()
        ...
        selected_indices = list(map(lambda x: x.strip(), input(
            "Enter the numbers of the users to assign as members (e.g.:'1,2') or press ENTER to go back: ").split(',')))
        if selected_indices[0] == "":
            return
    ...
    def remove_assignee_menu(self, task: Task, user: User):
        if user.ID != self.owner:
            console.print(
                "Only the project owner can remove assignees.", style="Error")
            return
        if not task.assignees:
            console.print(
                "There are no assignees for this task to remove.", style="Error")
            return
        clear_screen()
        ...
        selected_indices = list(map(lambda x: x.strip(), input(
            "Enter the numbers of the users to remove from task (e.g.:'1,2') or press ENTER to go back: ").split(',')))
        if selected_indices[0] == "":
            return
        ...
        task.add_to_history(
            user.ID, action="remove assignee", members=member_ID)

    def create_manage_task(self, user: User):
        if self.owner != user.ID:
            console.print(
                "Only the project owner can create tasks.", style="Error")
            wait_for_key_press()
            return
        while True:
            clear_screen()
            ...
            console.print(
                "Please provide the following details to create a new task:", style="Info")
            console.print("(or press ENTER to go back)", style="Info")
            ...
            logger.info(f"A new task [name : {new_task.title} , id : [{
                        new_task.ID}]] created by [{user.username}]")
            wait_for_key_press()
            break
    ...
    def delete_task(self, task: Task, user: User):
        if user.ID == self.owner:
            ...
            console.print(
                f"Task [id = {task.ID}] has deleted successfully", style='Notice')
            logger.debug(
                f"Task [id = {task.ID}] deleted by user [user = {user.username}]")
        else:
            console.print(
                "Only the project owner can delete project.", style="Error")
        wait_for_key_press()

    def view_project_tasks(self, user: User):
        if not self.tasks:
            console.print(
                "There are no tasks for this project.", style="Error")
            wait_for_key_press()
        else:
            while True:
                clear_screen()
                ...
                main_table.add_column(
                    "BACKLOG", style="cyan", justify="center", width=50)
                main_table.add_column(
                    "TODO", style="yellow", justify="center", width=50)
                main_table.add_column(
                    "DOING", style="magenta", justify="center", width=50)
                main_table.add_column(
                    "DONE", style="green", justify="center", width=50)
                main_table.add_column(
                    "ARCHIVED", style="blue", justify="center", width=50)
                ...
                backlog_table.add_column(
                    "Task title", justify="center", width=50)
                ...
                for task in self.tasks.values():
                    ...
                    if instance_task.status == "BACKLOG":
                        backlog_table.add_row(
                            instance_task.ID, instance_task.title)
                ...
                main_table.add_row(backlog_table if len(backlog_table.rows) != 0 else "No task Available", todo_table if len(todo_table.rows) != 0 else "No task Available", doing_table if len(
                    doing_table.rows) != 0 else "No task Available", done_table if len(done_table.rows) != 0 else "No task Available", archived_table if len(archived_table.rows) != 0 else "No task Available")

                console.print(main_table)

                task_id = input(
                    "Enter task ID to manage (or press ENTER to go back): ")
                if task_id == "":
                    return
                flag = False
                for task in self.tasks.values():
    ...
    def create_project(user: User):
        while True:
            ...
            if title:
                ...
                console.print(
                    "Title cannot be empty. Please enter a valid title.", style="Error")
                wait_for_key_press()

        project = Project(title, user.ID)
        project.save_project_data()
        logger.info(f"A new project [name : {project.title} , id : {
                    project.ID}] created by [{user.username}]")
    ...
    def delete_project(self, user: User):
        ...
        try:
            if os.path.exists(file_path):
                ...
                console.print(
                    f"Project '{self.title}' has been deleted successfully.", style="Notice")
                logger.info(
                    f"Project [id : {self.ID}] deleted by owner [user : {user.username}]")
                wait_for_key_press()
                return True
            else:
    ...
    def change_task_fields(self, user: User, task: Task):
        while True:
            ...
            task.view_task()
            console.print(
                "Which task field do you want to change?", style="Info")
            ...
            if choice == "1":
                if task.change_status():
                    task.add_to_history(
                        user.ID, action="change status", new_amount=task.status)
                    self.update_task(task)
                    self.save_project_data()
            elif choice == "2":
                if task.change_priority():
                    task.add_to_history(
                        user.ID, action="change priority", new_amount=task.priority)
                    self.update_task(task)
                    self.save_project_data()
            elif choice == "3":
                if task.change_start_time():
                    task.add_to_history(
                        user.ID, action="change start time", new_amount=task.start_time)
                    self.update_task(task)
                    self.save_project_data()
            elif choice == "4":
                if task.change_end_time():
                    task.add_to_history(
                        user.ID, action="change end time", new_amount=task.end_time)
                    self.update_task(task)
                    self.save_project_data()
            elif choice == "5":
                if task.change_title():
                    task.add_to_history(
                        user.ID, action="change title", new_amount=task.title)
                    self.update_task(task)
                    self.save_project_data()
            elif choice == "6":
                if task.change_description():
                    task.add_to_history(
                        user.ID, action="change description", new_amount=task.description)
                    self.update_task(task)
                    self.save_project_data()
            elif choice == "7":
                break
    ...
    def manage_task(self, user: User, task: Task):
        if user.ID not in task.assignees and user.ID != self.owner:
            console.print(
                "You don't have access to modify this task.", style='Error')
            wait_for_key_press()
            return
        ...
        while True:
            ...
            if choice == "1":
                ...
                if task.add_comment(user.ID, user.ID == self.owner):
                    task.add_to_history(
                        user.ID, action="add comment", message=task.comments[-1])
                    self.update_task(task)
                    self.save_project_data()
                ...
                if task.edit_comment(user):
                    task.add_to_history(
                        user.ID, action="edit comment", new_amount="VIEW EDITED MESSAGE IN VIEW COMMENTS")
                    self.update_task(task)
                    self.save_project_data()
                ...
                if task.remove_comment(user):
                    task.add_to_history(
                        user.ID, action="remove comment", new_amount="MESSAGE REMOVED")
                    self.update_task(task)
                    self.save_project_data()
    ...
    def view_user_projects(user: User):
        ...
        if data is not None and len(data['projects']) != 0:
            user_projects = [Project.load_project_data(
                proj_id) for proj_id in data["projects"]]

        else:
            console.print(
                "You don't have any projects to display.", style="Error")
        ...
        for i, project in enumerate(user_projects, start=1):
            table.add_row(
                str(i), project["ID"], project["title"], get_username(project["owner"]))
            project_map[str(i)] = project
        console.print(table)

        while True:
            project_number = input(
                "Enter project number to manage (or press ENTER to go back): ")
            if project_number == "":
                return
            if project_number in project_map:
                ...
                logger.debug(
                    f"User [{user.username}] is managing project [id: {project_instance.ID}]")
...
def main_menu():
    while True:
        ...
        console.print(
            "Already have an account? Login now. New user? Register to get started.", style="Info")
        ...
        if choice == "1":
            ...
            console.print(
                "Thank you for using the Project Management System. Have a great day!", style="Notice")
...
def user_menu(user: User):
    while True:
        ...
        if choice == "1":
            ...
            console.print(
                "You have been successfully logged out.", style="Notice")
