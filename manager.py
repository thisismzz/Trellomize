...
class Manager:
    ...
    def manager_menu(self):
        while True:
            ...
            if choice == '1':
                ...
                selected_usernames = list(map(lambda x: x.strip(), input(
                    "Enter usernames to deactive (format:'user1,user2') or 'back' to go back: ").split(',')))
                ...
                for username in all_usernames:
                    console.print("-", username)
                selected_usernames = list(map(lambda x: x.strip(), input(
                    "Enter usernames to activate (format:'user1,user2') or 'back' to go back: ").split(',')))
    ...
    def deactive_user(username):
        ...
        if user_data["active"] == False:
            console.print(
                f"User ({username}) has already been deactivated", style='Error')
            return
        ...
        console.print(
            f"User ({username}) has been deactivated successfully", style='Notice')
    ...
    def activate_user(username):
        ...
        console.print(
            f"User ({username}) has been activated successfully", style='Notice')
...
if __name__ == "__main__":
    ...
    create_admin_parser = subparsers.add_parser(
        "create-admin", help="Create admin credentials")
    create_admin_parser.add_argument(
        "--username", required=True, help="Admin username")
    create_admin_parser.add_argument(
